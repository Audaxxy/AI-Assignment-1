...
{
	public GameObject winScreen, loseScreen,UI;

	#region 3rd party movement code
	...
	void Update()
	{
		...
		if (Input.GetKey(KeyCode.E))
		{
			...
			if (Physics.Raycast(playerCamera.transform.position, playerCamera.transform.forward, out hit))
			{
				...
				if (door != null)
				{
					//If ray hits a door that is safe
					if (hit.transform.GetComponent<Door>().safe == true)
					{
					...
					//If ray hits an unsafe door
					else if (hit.transform.GetComponent<Door>().safe == false)
					{
